...
class ReRunResult(TestResult):
    ...
    def addFailure(self, test, err):
        ...
        if test.count < self.count:
            ...
            super().addFailure(test, err)
            if test.count != 0:
                sys.stderr.write("================rerun {} times over================\n".format(test.count))

    def addError(self, test, err):
        if not hasattr(test, 'count'):
            test.count = 0
            sys.old_stderr = sys.stderr
            sys.old_stderr.write("{}run——>[Error]\n".format(test))
            ...
            for string in traceback.format_exception(*err):
                sys.old_stderr.write(string)
                sys.stderr.write(string)
            sys.old_stderr.write("================{} reun {} times================\n".format(test, test.count))
            time.sleep(self.interval)
            test.run(self)
        else:
            super().addError(test, err)
