        ...
        try:
            ...
            while retries < 8:
                try:
                    ...
                    retries += 1
                    logger.error(f"Safemode status check failed: {err}")
                    if retries >= 8:
            ...
            try:
                ...
                if retries >= 5:
                    logger.info("Safemode login with maximum retries")
            except Exception as err:
                logger.error(f"Safemode login failed: {err}")
        except Exception as err:
            logger.error(f"Exception in login_safemode_from_ui_linux: {err}")

    def upgrade_rom(self):
        try:
            out = subprocess.getoutput('ping 192.168.168.168 -c 2')
            if '100% packet loss' not in out:
                ...
                if self.compare_ver():
                    ...
                    time.sleep(5)
                    logger.info("Sleep 60s wait for the page reload.")
                    time.sleep(20)
                ...
                return False, "At first, it can't ping 192.168.168.168, or ssh is not enabled."
        except Exception as err:
            logger.error("upgrade_rom exception: " + str(err))
