...
class NetworkDevice:
    ...
    def __init__(self, ip, console_user='qa', console_password='qa', user='admin', password='password', new_password='password', wrong_password='Admin@123456789', soniccore_user='techsupport', soniccore_pwd='sonicwall-514', device='none', console_ip='', console_port='', supported_config_mode='cli-ssh', **kwargs):
        ...
        if self.console_ip == '10.6.0.37':
            self.soniccore_pwd ='sonicwall-512'
...
class Host(NetworkDevice):
    def __init__(self, ip, user='root', password='password'):
        super().__init__(ip, user=user, password=password)
        if self.ip.lower() != 'localhost':
            for i in range(0,2):
                try:
                    ...
                    logger.info(f'Fail to ssh to {ip}:{e}')
    ...
    def start_DHCPv4_server(self, conf_file='/etc/dhcp/dhcpd.conf'):
        ...
        if re.search('dhcpd.*running',result, re.I):
            logger.info('start dhcpd service sucessfully.')
            return True
        logger.error('start dhcpd service unsucessfully.')
        return False

    def stop_DHCPv4_server(self, conf_file='/etc/dhcp/dhcpd.conf'):
        logger.info('Stop dhcpd service')
        self.send_command( f"service dhcpd stop")  
        logger.info("Move back /etc/dhcp/dhcpd.conf")
    ...
    def start_DHCP_server(self):
        ...
        if re.search(r'dibbler-server start', result, re.I):
            logger.info('dibbler server start successfully.')
            return True
        return False
    ...
    def stop_DHCP_server(self):
        ...
        if pid:
            ...
            logger.info('dibbler server has been killed')
        return True
            
    def start_PPPoE_server(self, interface=None, local_ip=None, assign_ip=None, ppp_secrets="/etc/ppp/pap-secrets", pppoe_option="/etc/ppp/pppoe-server-options"):
        if not interface or not local_ip or not assign_ip:
            logger.error("Should define interface,local ip,assign ip for pppoe server!")
            return False
        logger.info("Backup /etc/ppp/pap-secrets and /etc/ppp/pppoe-server-options")
        ...
        logger.info(f"Copy {ppp_secrets} {pppoe_option} to /etc/ppp")
        ...
        logger.info(f"Start PPPoE server...cmd:pppoe-server -I {interface} -L {local_ip} -R {assign_ip} -N 5")
    ...
    def stop_PPPoE_server(self):
        logger.info("Stop PPPoE server.")
        self.send_command( "pkill pppoe-server")
        logger.info("Restore /etc/ppp/options.pptpd,/etc/ppp/pap-secrets")
    ...
    def start_HTTPS_server(self, conf_path=None):
        if not conf_path:
            logger.error('Please specify conf_path for your https server.')
            return False
        self.send_command(f"cp -rf {conf_path}/nginx/nginx-1.11.5 /usr/local/")
        self.send_command(f"cd /usr/local/nginx-1.11.5;pwd;bash ./configure --with-http_ssl_module --without-http_rewrite_module;make;make install")
        ...
        logger.info("------Make sure https server start on target PC.")
        result = self.send_command("netstat -nlp|grep :443|awk {'print $7'}")
        if not result:
            logger.error('start nginx failed: ')
            return False
        return True
    ...
    def get_exe_lnx(self, exe_name, cmd):
        ...
        try:
            ...
            logger.error(f'Does not find {exe_name}')
    ...
    def config_IPv6_ip(self, ip=None, prefix=None, interface=None):
        try:
            ...
            logger.error(f'Send command fail: {e}')
        return False
        
    def delete_IPv6_ip(self, ip=None, prefix=None, interface=None):
        try:
            return self.send_command(f'ifconfig {interface}  inet6 del {ip}/{prefix}')
        except Exception as e:
            logger.error(f'Send command fail: {e}')
        return False
    ...
    def ping(self, ip, size=64, num=5):
        result = self.send_command(f'ping -c {num} {ip} -l {size}')
        logger.info(result)
    ...
    def ping6(self, ip, num=5):
        ...
        logger.info(result)
        if re.search("unreachable", result, re.I) or re.search("100% packet loss", result, re.I):
            return False
        else:
            return True
            
    def send_command(self, cmd, backend=False, run_async=False):
