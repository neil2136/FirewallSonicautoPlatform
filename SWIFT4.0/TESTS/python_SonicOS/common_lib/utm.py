...
def enable_ssh(ip):
    ...
    commands = ['configure', 'dns server inherit','administration','max-login-attempts-cli 10', 'commit', 'end']
    ui_obj = FWPage()
    for each in range(0,5):
        if ui_obj.enable_ssh_for_x0_interface():
            logger.info('Change DNS to inherit') 
            fw.do_cli_commands(commands)
            return True
        logger.info('Sleep 5s for try enable ssh again.')
        time.sleep(5)

    return False

def is_Firewall_up(ip='192.168.168.168', max=80, console_ip='', console_port='',console_user='qa', console_password='qa', ssh=True, mode='ping',mode_port=443,pre_sleep=240,reboot_check=False):
    global G_PASSWORD
    if pre_sleep > 0:
        logger.info(f'Sleep {pre_sleep}s for firewall up.')
    ...
    for each in range(0,max):
        ...
        if mode == 'ping':
           ...
           output = not tcpping(ip, mode_port)
            ...
            fw_ssh = FirewallCLI(ip, user='admin', console_ip=console_ip,console_port=console_port,password='password', supported_config_mode='cli-console')
            output,rc = fw_ssh.pre_console_connect(1)
            if rc == 'Could not start telnet to conserver':
                return False
            output = not output
            # if not output:
            #     status=fw_ssh.do_cli_command('show version')
            #     if 'NSsp 15700' in status:
            #         commands=['configure','security-policy ipv4 uuid 0000','name any-to-X0','source address any','destination address name X0\ IP','action allow','action-profile Default\ Profile','commit','interface X0','management https','management ping','commit','end','end']
        ...
        if not output:
            ...
            if ssh:
                cmd = 'newman run ' + os.environ["PYTHON_COMMON_HOME"] + '/config/enable_ssh.json -k --disable-unicode --delay-request 1200'
                cmd1= 'newman run ' + os.environ["PYTHON_COMMON_HOME"] + '/config/enable_ssh_newpass.json -k --disable-unicode --delay-request 1200'
                for i in range(0,2):
                    logger.info("enable ssh by api\n")
                    logger.info(cmd)
                    cmd_return = subprocess.Popen(cmd, shell = True, stdout = subprocess.PIPE).communicate(timeout=100)[0].decode('ASCII')
                    if re.search(r'Login Your default password must be changed', cmd_return, re.I):
                        ...
                        change =change_password(new_password=G_PASSWORD_NEW)
                        if not change:
                            rc = 1
                        else:
                            G_PASSWORD = True # True means default password change to S0nic@uto
                            logger.info(f'is_firewall_up: ')
                            logger.info(G_PASSWORD)
                            logger.info(cmd1)
                            rc = os.system(cmd1)
                            if rc != 0:
                                rc = os.system(cmd1)
                        ...
                        rc = os.system(cmd1)
                        if rc != 0:
                            logger.debug('Enable use new password once again.')
                            if not re.search('error',os.popen(cmd1).read(),re.I):
                                rc = 0
                    else:
                        G_PASSWORD = None
                    time.sleep(10)
                    ...
                    if G_PASSWORD:
                        fw.new_password = Params.G_NEW_PASSWORD
                    #####1
                    commands = ['configure', 'dns server inherit', 'administration','max-login-attempts-cli 10','exit','commit','log categories','logging-level debug','commit','end','end']
                    logger.info('Change DNS to inherit')
                    rc1 = fw.do_cli_commands(commands)
                    if rc1:
                        break
                    else:
                        logger.debug(cmd_return)
            if (rc !=0 or not rc1):
                return False
            return True
        logger.info('Sleep {}s for firewall up.'.format(sleep_time*2))
        time.sleep(sleep_time*2)
        if each == max-1:
            logger.error('Firewall not up after {}s'.format((each+1)*sleep_time))
            return False
    return True
...
def is_ipv4(ip):
    try:
        ...
        ValueError
        logger.info(ip, " is not an IPv4 addr")


class FirewallCLI(NetworkDevice):
    def __init__ (self, ip,  prompt='[\w|\d]+\)?>', cli_port='22', cli_encr='', cli_compr=False, mac_spec='', option='',
                 other='', ssh_version=2, logfile='/tmp/snwlssh.log',user_type='system',boot_check=True,**kwargs):
    ...
    def pre_console_connect(self, errcode=0):
        ...
        if not port_prefix:
            ...
            if not self.ssh.expect([pexpect.TIMEOUT, 'login:']):
                logger.error('Unable to Telnet to: ' + self.console_ip + ' ' + str(self.console_port))
                if errcode:
                    return False, 'Unable to telnet to conserver'
                return False
            self.ssh.sendline(self.options['console_user'])
            if not self.ssh.expect([pexpect.TIMEOUT, 'Password:']):
                if errcode:
                    return False, 'Could not get conserver Password prompt'
                return False
            self.ssh.sendline(self.options['console_password'])
        ...
        #                '(yes\/no)', 
            ...
            try:
                ...
                logger.debug('console output {} {} not meet the expected match.'.format(self.ssh.before, self.ssh.after))
            ...
            if index == 0: # Timeout
                ...
                logger.info('Console to dut(user:) via telnet {} {} sucessfully.'.format(self.console_ip,self.console_port))
                if errcode:
                    return True, 'Console to dut via telnet successfully'
                return True
            elif index == 3:
                self.ssh.sendline('smconsole 1')
                self.ssh.sendline('')
                continue
            elif index ==2 or index == 4 or index ==5 or index ==6 or index == 16 or index==15 or index == 17 or index ==18:
                ...
                self.ssh.sendline(self.soniccore_user)
                rc = 2
                break
            elif index == 14:
                logger.info('Maximum login attempts exceeded: ' + 'telnet ' +  self.console_ip + ' ' + str(self.console_port))
                if errcode:
                    return False, 'Maximum login attempts exceeded'
                return False
            continue
    ...
    def console_connect(self, errcode=0):
        ...
        #                '(yes\/no)', 
            ...
            continue
        if rc == 2:
            logger.info('Enter soniccore mode...')
            while True:
                index = self.ssh.expect([
                    pexpect.TIMEOUT, 
        #                '(yes\/no)', 
                ...
                if index == 0: # Timeout
                    ...
                    self.ssh.sendline(self.soniccore_pwd)
                    continue
                elif index == 2:
                    self.ssh.sendline('smconsole 1')  
                    self.ssh.sendline('')
    ...
    def do_cli_command(self, command, customPrompt='>#', timeout=240):
        ...
        while True:
            ...
            if index == 0: # Timeout
                logger.info('Wrong command: ' + bytes.decode(self.ssh.before) + str(self.ssh.after))
                return False
            elif index == 1:
                self.ssh.send(' ')
                continue
                ...
                logger.error('Command ' + '\'' + command + '\''+ ' has error')
                ...
                self.flushbuffer()
                logger.info('======>' + output)
                if Params.sonicos_ver != '7.1.1_cover_x86':
                    self.ssh.sendline('yes')
                    logger.info('Firewall need restart.')
                    if not is_Firewall_up(ip=self.ip, ssh=False):
                        output = output + 'Error: reboot firewall fail.'
                if re.search('console', self.supported_config_mode, re.I):
                    self.ssh.send('')
                    continue
                self.ssh.close()
                ...
                self.flushbuffer()
                logger.info('=>' + output)
                logger.info('Firewall need factory default.')
                self.flushbuffer()
            #    is_Firewall_up(ip=self.ip, ssh=True)
                mode = 'ping'
                if re.search('console', self.supported_config_mode, re.I):
                    mode='console' 
                if Params.sonicos_ver != '7.1.1_cover_x86' and self.boot_check:  
                    if not is_Firewall_up(ip=self.ip, ssh=False,console_ip=self.console_ip,console_port=self.console_port,mode=mode):
                        output = output + 'Error: restore firewall fail.'
                if re.search('console', self.supported_config_mode, re.I):
                    self.ssh.sendline('')
                    continue
                self.ssh.close()
    ...
    def do_cli_commands(self, commands, tag=0, timeout=200):
        for i in range(0,2):
        ...
        for command in commands:
            ...
            if tmp:
                ...
                if re.match(r'exiting \(yes\/no\/cancel\)', tmp, re.I):
                    self.ssh.sendline('yes')
                elif (re.search(r'Restart', tmp, re.I|re.M) or re.search(r'Restoring to factory defaults', tmp, re.I|re.M)) and not re.search(r'Restart Required:', tmp):
                    if 'Error:' in output:
                        result = False
                    else:
                        result = True
    ...
    def console_connect_factory_reset(self, errcode=0):
        lines = ''
        output = ''
        prompt = ':'
        cmd = 'ssh -l ' + 'console' + ' ' + '10.5.64.103' + ' ' + '-p' + ' ' + self.cli_port
        print(cmd)
        self.ssh = pexpect.spawn(cmd)
        enter_password = 0
        while True:
                try:
                        ...
                        print("could not start ssh console")
                if index == 1:
                # SSH does not have the public key. Just accept it.
                        self.ssh.sendline('yes')
                        self.ssh.expect('yes')
                        print("Accepted ssh public key")
                        ...
                        print(self.ssh.before)
                        self.ssh.expect(':')
                        self.ssh.sendline(Params.G_NEW_PASSWORD)
                        print(self.ssh.before)
                        self.ssh.expect(':')
                        self.ssh.sendline(Params.G_NEW_PASSWORD)
                        print(self.ssh.before)
                        self.ssh.expect('>')
                        time.sleep(5)
                        self.ssh.sendline('show interface X1')
                        time.sleep(2)
                        self.ssh.sendline(' ')
                        self.ssh.expect('>')
                        output += bytes.decode(self.ssh.before) + bytes.decode(self.ssh.after)
                        ...
                        line = output.split('\n')
                        lines = line[0:4]
                        print("Succesfully logged into the Firewall")
                        break
        return lines

    def do_cli_command_with_ip(self, command, customPrompt='>#', timeout=240, change_ip=False, console_ip=None, console_port=None):
        '''send a single command'''
        logger.info('Send command:' + command)
        self.flushbuffer()
        output = ""
        ...
        while True:
            ...
            #    is_Firewall_up(ip=self.ip, ssh=True)
                ...
                if re.search('console', self.supported_config_mode, re.I):
                    mode='console'
                if change_ip:
                    is_Firewall_up(ip=console_ip, ssh=False,console_ip=console_ip,console_port=console_port,mode=mode)
                else:
                    is_Firewall_up(ip=self.ip, ssh=False,console_ip=self.console_ip,console_port=self.console_port,mode=mode)
                if re.search('console', self.supported_config_mode, re.I):
                    self.ssh.sendline('')
                    continue
                self.ssh.close()
    ...
    def do_cli_commands_with_ip(self, commands, tag=0, timeout=200, change_ip=False, console_ip=None, console_port=None):
        for i in range(0,2):
            rc, output = self.cli_login(1)
            if rc:
                break
        ...
        output = ''
        for command in commands:
            tmp = self.do_cli_command_with_ip(command, timeout=timeout, change_ip=change_ip, console_ip=console_ip, console_port=console_port)
            if tmp:
                output = output + tmp
                if re.match(r'exiting \(yes\/no\/cancel\)', tmp, re.I):
                    self.ssh.sendline('yes')
        ...
        if (tag and key in kwargs.keys()) or not tag:
            if key in kwargs.keys():
                if isinstance(kwargs[key], bool):
                    ...
                    logger.error('Make sure {} is valid'.format(kwargs[key]))
                    return False
            else:
                pass
        else:
            logger.error('Key {} Must be specified!'.format(key))
            return False 
        return True
...
class FirewallAPI(NetworkDevice):
    ...
    def __init__ (self, ip,  prompt='>', headers=headers1, port=443, logfile='/tmp/snwlapi.log',user_type='system',manage=True, **kwargs):
        super().__init__(ip, **kwargs)
        self.logfile = logfile  
        self.user_type = user_type
    ...
    def api_login(self, check_login=True):
        if self.user_type == 'system':
            if self.is_api_login_valid():
                logger.info('Already login in, returning success.')
                return True    
        payload = {"override": True}
        payload = json.dumps(payload)
        url = 'https://' + self.ip + '/api/sonicos/auth'
        urllib3.disable_warnings()
        resp = requests.post(url, auth=HTTPDigestAuth(self.user, self.new_password), data=payload, headers=self.headers, verify=False)
        ...
        try:
            ...
            if re.search(r'Your default password must be changed at first time login', login_response['status']['info'][0]['message'], re.I):
                change =change_password(new_password=G_PASSWORD_NEW)
                if not change:
                    logger.error('change password failed.')
                    return False
                    ...
                    G_PASSWORD = True
                    self.new_password = G_PASSWORD_NEW
                    return self.api_login(check_login)
            elif login_response['status']['success']:
                logger.info('Successfully login sonincos')   
                time.sleep(3)
                if check_login and self.user_type == 'system':
                    if self.is_api_login_valid():
                        return True
                    else:
                        return False
                elif self.user_type != 'system' and self.manage:
                    if self.api_start_management():
                        self.is_api_login_valid()
                        return True
                    else:
                        return False
                return True
            elif re.search(r'Incorrect name\W+password', login_response['status']['info'][0]['message'], re.I):
                # match 'Incorrect name/password'
                logger.info(f'Old password fail to login, try use {G_PASSWORD_NEW}.')
                self.new_password = G_PASSWORD_NEW
    ...
    def api_start_management(self):
        url = 'https://' + self.ip + '/api/sonicos/start-management'
        urllib3.disable_warnings()
        resp = requests.post(url, auth=HTTPDigestAuth(self.user, self.new_password), headers=self.headers, verify=False)
        response = resp.content.decode('utf-8')
        logger.info("Start management response after decode:\r\n{}".format(response))
        try:
            login_response = json.loads(response)
            logger.debug(login_response)

            if login_response['status']['success']:
                logger.info('Successfully start management.')
            else:
                logger.info("Login error\nResponse is " + login_response['status']['info'][0]['message'])
                return "Start management error\nResponse is " + login_response['status']['info'][0]['message']
        except:
            logger.error('Start management failed')
            return False
        return True
    ...
    def is_api_login_valid(self):
        ...
        try:
            ...
            if re.search(r'status.*Non-config mode',return_msg,re.I) or re.search(r'status.*Non config mode',return_msg,re.I):
                logger.debug('Firewall in non-config mode, try switch to config mode.')
                #self.api_post('api/sonicos/config-mode')
                requests.post('https://' + self.ip + '/api/sonicos/config-mode', headers=self.headers, data=None, params=None, timeout=60, verify=False)
                return True
            elif re.search(r'[\(\"]config[\s_]mode|\W config[\s_]mode',return_msg,re.I):
                logger.info('Already in config mode.')
                return True
            else:
                logger.info(f'Fail to get {url}, try re-login')
                return False
        except Exception as e:
            logger.error("GET request is not successful {}".format(e))
            return False

    def api_get(self, url, params=None, nologin=False, log_switch=True, timeout=60):
        ...
        if not nologin:
            self.api_login()
        try:
            response = requests.get(url, headers=self.headers, params=params, timeout=timeout, verify=False)
        except Exception as e:
            logger.error("GET request is not successful {}".format(e))
            return False
        resp = response.content.decode('utf-8',"ignore")
    ...
    def api_get_extranet(self, url, params=None, nologin=False, log_switch=True):
        # url = 'https://' + str(self.ip) + '/' + url
        return_msg = {}
        if not nologin:
            self.api_login()
    ...
    def api_put(self, url, msg=False, data=None, params=None, headers=headers1,nologin=False, check_online='ping', files=None):       
        if not nologin:
            self.api_login()
        url = 'https://' + str(self.ip) + '/' + url
        logger.info(url)
        logger.info('using api_put'.center(130,'='))
        try:
            if files is not None: 
                response = requests.put(url, headers=headers, files=files, params=params, timeout=60, verify=False)
            else:
                logger.info(json.dumps(data, indent=1))
                if data:
            ...
            logger.error("POST request is not successful {}".format(e))
            return False
        ...
        try:
            ...
            logger.error("Fail to get ['status']['cli']['pending_config'] in response")
        try:
            if return_msg['status']['cli']['restart_required'] and\
                return_msg['status']['cli']['restart_required'].lower() == 'true':
                logger.info( 'Restart needed as per response, so restarting firewall')
                if Params.sonicos_ver != '7.1.1_cover_x86':
                    return_value &= self.api_restart(check_online=check_online)
        except KeyError as reason:
            logger.debug("Not get key return_msg['status']['cli']['restart_required']")
        if msg:
    ...
    def api_post(self, url, msg=False, data=None, params=None, headers=headers1, nologin=False, check_online='ping' ):
        ...
        url = 'https://' + str(self.ip) + '/' + url
        logger.info(url)
        logger.info('using api_post'.center(130,'='))
        try:
            logger.info(json.dumps(data, indent=1))
            ...
            logger.info("ERR:POST request is not successful {}".format(e))
            return False
        status_code = response.status_code
        logger.info("status code:" + str(status_code))
        if status_code == 200:
    ...
    # Add the loop parameter to control the number of loops instead of the hardcode 10 in method
    def api_post_no_need_pending(self, url, msg=False, data=None, params=None, headers=headers1, nologin=False, sleep_time=5, loop=10):
        if not nologin:
            self.api_login()
        url = 'https://' + str(self.ip) + '/' + url
        logger.info(url)
...
#            self.api_login()
        ...
        try:
            ...
            logger.info(f'Pending changes is: {return_value}')
            if return_msg['status']['info'][0]['message'] == 'Non config mode' or return_msg['status']['info'][0]['message'] == 'Not allowed in current mode':
    ...
    def api_delete(self, url, msg=False, data=None, params=None, nologin=False):
        if not nologin:
            self.api_login()
        url = 'https://' + str(self.ip) + '/' + url
        logger.info(url)
        logger.info('using api_delete'.center(130,'='))
        try:
            logger.info(json.dumps(data,indent=1))
            if data:
                data = json.dumps(data)
        ...
        logger.info(return_msg)
        if msg:
            pending_value, pending_msg = self.api_post_pendingchanges(msg=msg, nologin=nologin) 
        else:
            pending_value = self.api_post_pendingchanges(msg=msg, nologin=nologin) 
        try:
            if return_msg['status']['cli']['restart_required'] and\
                return_msg['status']['cli']['restart_required'].lower() == 'true':
                logger.info( 'Restart needed as per response, so restarting firewall')
                if Params.sonicos_ver != '7.1.1_cover_x86':  
                    return_value &= self.api_restart()
        except KeyError as reason:
            logger.debug("Not get key return_msg['status']['cli']['restart_required']")
        if msg:
            if not pending_value:
    ...
    def api_restart(self, check_online='ping'):
        ...
        if status_code == 200:
            ...
            logger.info('Restart fail, error code:{}'.format(status_code))
            return False
...
# if __name__ == "__main__":
    ...
    # fw.cgi_login()
    #fw_local_user = Firewall(ip, user='user_test1', new_password='sonicwall', supported_config_mode='api', user_type='local', manage=False)#manage means if user can manage firewall,
    #fw_local_user.api_login()
