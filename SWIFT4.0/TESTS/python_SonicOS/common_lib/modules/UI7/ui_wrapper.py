...
class Browser:
    ...
    def get_browser(self):
        ...
        try:
            ...
            if browser_type == "chrome":
                ...
                self.browser = webdriver.Chrome(webdriver_directory + "\chromedriver.exe", chrome_options=chrome_options)
                #self.delete_browser_history()
    ...
    def check_and_accept_multiple_alerts(self):
        try:
            self.wait_for_page_data_to_be_rendered()
            iteration = 0
            while iteration < 2:
                while self.does_element_exist_now('xpath', '//div[contains(@class, "sw-status-info")]'):
                    self.accept_alert()
                iteration = iteration + 1
            return True
        except Exception as err:
            logger.error(str(self.code_flow) + "Failed to accept multi alert")
            logger.error(str(self.code_flow) + "Exception \t: " + str(err))
            Assertion.fail(str(self.code_flow) + "Failed to accept multi alert")

    def configure_toggle_button(self, toggle_label, configure_value, group_name=None, modal=False, label=True, position=None, preceding=False):
        try:
            self.wait_for_page_data_to_be_rendered()
            modal_xpath = '//div[contains(@class, "sw-modal__main-body")]'
            togglelabelxpath = '*[contains(@class,"label")]'
            ...
            togglelabel_xpath4 = '/following::*[contains(text(), "' + toggle_label + '")]'
            groupname_xpath = [groupname_xpath1, groupname_xpath2]
            togglelabel_xpath = [togglelabel_xpath1, togglelabel_xpath2, togglelabel_xpath3, togglelabel_xpath4]

            if preceding is True:
                togglebutton_xpath = '/preceding-sibling::*[contains(@class,"sw-toggle")]'
            else:
                togglebutton_xpath = '/following::*[contains(@class,"sw-toggle")]'
            xpath_list = []
            xpath1 = modal_xpath + '//self::*[contains(text(),"' + toggle_label + '")]' + togglebutton_xpath
            xpath_list.append(xpath1)
            ...
            for xpath in xpath_list:
                if self.does_element_exist_now('xpath', xpath):
                    ...
                    if element_status != configure_value:
                        element.click()
                    return True
            raise Exception("None of the above XPATH matched")
        except Exception as err:
            logger.error(str(self.code_flow) + "Failed to configure toggle button")
            logger.error(str(self.code_flow) + "Configure toggle button XPATH : " + str(xpath_list))
            logger.error(str(self.code_flow) + "Exception \t: " + str(err))
            Assertion.fail(str(self.code_flow) + "Failed to configure toggle button")

    def wait_for_page_data_to_be_rendered(self):
        try:
            iteration = 0
            while iteration < 2:
                if not self.does_element_exist_now('xpath', '//div[contains(@class, "sw-blocking-progress")]'):
                    iteration = iteration + 1
            return True
        except Exception as err:
            logger.error(str(self.code_flow) + "Wait for page data to be rendered verification failed")
            logger.error(str(self.code_flow) + "Exception \t: " + str(err))
            Assertion.fail(str(self.code_flow) + "Wait for page data to be rendered verification failed")
    ...
    def did_page_load_successfully(self):
        ...
        for attempt in range(5):
            try:
                ...
                logger.info(str(self.code_flow) + "Caught JavascriptException exception. Retrying to get page load status...")
                continue
            if attempt == 5:
                return False
