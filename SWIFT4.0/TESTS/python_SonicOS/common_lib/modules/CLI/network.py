...
class WebproxyCli:
    ...
    def config_webproxy(self, tag=0, **kwargs):
        ...
        if ('server' in kwargs.keys() and 
            ...
            # error cmd in current fw console
    ...
    def del_user_proxy_server(self, proxy_ip):
        commands = ['configure', 'web-proxy']
        if proxy_ip:
            commands.append('no user-proxy-server ' + proxy_ip)
        else:
            logger.info('input proxy ip is empty.')
            return False
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
...
class InterfaceCli:
    ...
    def config_one_arm_interface(self, interface='X2', peer='192.168.20.222'):
        commands = [
            'configure',
    ...
    def get_interface_mac(self, interface='X0'):
        commands = ['show interface ' + interface + ' mac']
        output = self.fw.do_cli_commands(commands, tag=1)[1]
        return output
    ...
    def shutdown_interface(self, interface=None):    
        commands = ['configure', 'interface ' + interface, 'shutdown-port']
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result

    def no_shutdown_interface(self, interface=None):    
        commands = ['configure', 'interface ' + interface, 'no shutdown-port']
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result
    ...
    def del_all_vlan_interfaces(self):
        commands = ['configure', 'no interfaces vlan', 'commit', 'exit']
        result = self.fw.do_cli_commands(commands)
        return result

    def add_tunnel_interface_4to6(self, add_tag=False, **kwargs):
        commands = ['configure']
        self.fw._is_key_exist(commands, kwargs, 'name', key_new='tunnel-interface 4to6', tag=True)

        if 'type' not in kwargs.keys():
            logger.error("Please specify type of one of dslite, 'v6plus or gre4to6")
            return False

        if kwargs['type'] in ['dslite', 'gre4to6', 'v6plus']:
            commands.append('type ' + kwargs['type'])
            commands_if = self._get_tunnel_interface_4to6(**kwargs)
        else:
            logger.error('{} is not one of dslite, v6plus, gre4to6'.format(kwargs['type']))
        commands.extend(commands_if)
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        if add_tag:
    ...
    def edit_v6plus_tunnel_interface_by_name(self, **kwargs):
        '''
        opt = {
            'name': 'test',
            'local-ipv6': "100::100"
        }
        key <name> must be specified, others are optional
        '''
        if 'name' not in kwargs.keys():
            logger.error('Please specify the tunnel name.')
            return False    
        commands = ['configure', 'tunnel-interface 4to6 ' + kwargs['name']]
        self.fw._is_key_exist(commands, kwargs, 'flow-reporting')
        self.fw._is_key_exist(commands, kwargs, 'fragment-packets')
        self.fw._is_key_exist(commands, kwargs, 'ignore-df-bit')
        self.fw._is_key_exist(commands, kwargs, 'send-icmp-fragmentation')
        commands.append('type v6plus')
        self.fw._is_key_exist(commands, kwargs, 'bound-if', key_new='bound-to interface')
        self.fw._is_key_exist(commands, kwargs, 'local-ipv6', key_new='local ipv6' )
        self.fw._is_key_exist(commands, kwargs, 'remote-ipv6', key_new='remote ipv6')
        self.fw._is_key_exist(commands, kwargs, 'comment')
        self.fw._is_key_exist(commands, kwargs, 'update-url-period')
        self.fw._is_key_exist(commands, kwargs, 'update-url')
        commands.extend(['end', 'commit', 'exit'])
    ...
    def _get_interface_lan_static(self, **kwargs):
        ...
        self.fw._is_key_exist(commands_static,kwargs, 'management https')
        self.fw._is_key_exist(commands_static,kwargs, 'management ping')
        self.fw._is_key_exist(commands_static,kwargs, 'management snmp')
        self.fw._is_key_exist(commands_static,kwargs, 'management ssh')
        self.fw._is_key_exist(commands_static,kwargs, 'user_login_http', key_new = 'user-login http')
        self.fw._is_key_exist(commands_static,kwargs, 'user_login_https',key_new = 'user-login https')
        self.fw._is_key_exist(commands_static,kwargs, 'shutdown-port')
        self.fw._is_key_exist(commands_static,kwargs, 'auto-discovery')
        self.fw._is_key_exist(commands_static,kwargs, 'multicast')
        self.fw._is_key_exist(commands_static,kwargs, 'cos-8021p')
        self.fw._is_key_exist(commands_static,kwargs, 'exclude-route')
        self.fw._is_key_exist(commands_static,kwargs, 'link-speed')
        self.fw._is_key_exist(commands_static,kwargs, 'mac')
        self.fw._is_key_exist(commands_static,kwargs, 'flow-reporting')
        self.fw._is_key_exist(commands_static,kwargs, 'mtu') 
        self.fw._is_key_exist(commands_static,kwargs, 'management-traffic-only')
        self.fw._is_key_exist(commands_static,kwargs, 'asymmertric-route')
        self.fw._is_key_exist(commands_static,kwargs, 'fqdn-assignment',key_new = 'management fqdn-assignment')
        return commands_static
    ...
    def _get_interface_management(self, **kwargs):
        ...
        if 'https-source' in kwargs.keys():
            commands_mgmt.append(f'management https-source {kwargs["https-source"]}')
        if 'ping-source' in kwargs.keys():
            commands_mgmt.append(f'management ping-source {kwargs["ping-source"]}')
        if 'snmp-source' in kwargs.keys():
            commands_mgmt.append(f'management snmp-source {kwargs["snmp-source"]}')
        if 'ssh-source' in kwargs.keys():
            commands_mgmt.append(f'management ssh-source {kwargs["ssh-source"]}')
    ...
    def _get_interface_user(self, **kwargs):
        ...
        self.fw._is_key_exist(commands_user, kwargs, 'user-http', key_new='user-login http')
    ...
    def _get_router_advertisement(self, **kwargs):
        ...
        if kwargs['router-advertisement']:
            ...
            # commands_ra.append('exit')
            if 'add_prefix' in kwargs.keys():
                commands_ra.append('prefix ' + kwargs['add_prefix']['ip'])
                self.fw._is_key_exist(commands_ra, kwargs['add_prefix'], 'autonomous')
                self.fw._is_key_exist(commands_ra, kwargs['add_prefix'], 'on-link')
                self.fw._is_key_exist(commands_ra, kwargs['add_prefix'], 'valid-lifetime')
                self.fw._is_key_exist(commands_ra, kwargs['add_prefix'], 'preferred', key_new='preferred lifetime')
                commands_ra.append('exit')
            if 'del_prefix' in kwargs.keys():
                commands_ra.append('no prefix ' + kwargs['add_prefix']['ip'])
            commands_ra.append('exit')
        else:
    ...
    def click_dhcpv6_release(self, interface):
        ...
        commands = ['configure', 'interface ipv6 ' + interface, 'ip-assignment dhcpv6']
        commands.append('release')
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
    ...
    def click_pppoe_reconnect(self, interface):
        commands = ['configure', 'interface ' + interface, 'ip-assignment '+ 'WAN'+ ' ' + 'pppoe']
        commands.append('reconnec 1')
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result
    ...
    def show_interface_status(self, interface=None, version='ipv4'):
        if not interface:
    ...
    def show_interface_prefix(self, iface=None):
        if not iface:
            logger.error('interface must be specified')
            return False
        command = [f'show interface ipv6 {iface} prefixes']
    ...
    def _get_natpolicy(**kwargs): 
        ...
        if ('orig_source_type' in kwargs.keys() and 
            ...
            if 'orig_source' in kwargs.keys() and kwargs['orig_source']:
                ...
                return False
        if 'orig_source_type' in kwargs.keys() and kwargs['orig_source_type'] == 'any': 
            command += 'source any '
        # translated parameter is 'original' instead of 'any'
        if ('trans_source_type' in kwargs.keys() and 
            ...
            if ('trans_source' in kwargs.keys() and kwargs['trans_source']):
                ...
                return False 
        if 'trans_source_type' in kwargs.keys() and (kwargs['trans_source_type'] == 'original'): 
        ...
        if ver.lower() == 'nat64':
            if 'trans_source_type' in kwargs.keys() and kwargs['trans_source_type']: 
                if kwargs['trans_source_type'] == 'original':
                    ...
                    return False
            if 'pref64_type' in kwargs.keys() and kwargs['pref64_type']:
            ...
            if ('orig_dest_type' in kwargs.keys() and kwargs['orig_dest_type'] == 'any'):
            ...
            if 'trans_dest_type' in kwargs.keys() and (kwargs['trans_dest_type'] != 'original' and kwargs['trans_dest_type']):
                if 'trans_dest' in kwargs.keys() and kwargs['trans_dest']:
                    ...
                    logger.error('Must specified trans_dest when trans_dest_type is not original.')
                    return False
            if 'trans_dest_type' in kwargs.keys() and kwargs['trans_dest_type'] == 'original': 
                command += 'translated-destination original '
            ...
            if 'orig_service_type' in kwargs.keys() and (kwargs['orig_service_type'] != 'any' and kwargs['orig_service_type']):
                if 'orig_service' in kwargs.keys() and kwargs['orig_service']:
                    ...
                    return False
            if 'orig_service_type' in kwargs.keys() and kwargs['orig_service_type'] == 'any': 
                command += 'service any '  
            ...
            if 'trans_service_type' in kwargs.keys() and (kwargs['trans_service_type'] != 'original' and kwargs['trans_service_type']):
                if 'trans_service' in kwargs.keys() and kwargs['trans_service']:
                    ...
                    logger.error('Must specified trans_service when trans_service_type is not original.')
                    return False
                if 'trans_service_type' in kwargs.keys() and kwargs['trans_service_type'] == 'original': 
    ...
    def _get_natpolicy_edit(**kwargs):
        ...
        if 'trans_source_new' in kwargs.keys():
            if 'trans_source_type_new' in kwargs.keys():
                commands.append('translated-source ' + kwargs['trans_source_type_new'] + ' ' + kwargs['trans_source_new'])
            else:
...
class DhcpServerCli:
    ...
    def show_dhcpserver_scope_v6(self, scope_name, mode):  # mode: dynamic,static
        commands = ['show dhcp-server ipv6 scope ' + mode + ' ' + scope_name]
        output = self.fw.do_cli_commands(commands, tag=1)[1]

    def show_dhcpserver_leases_statistic(self, version=''):  # version=ipv4 or ipv6
        if version:
            commands = ['show dhcp-server ' + version + ' leases statistic']
        else:
            commands = ['show dhcp-server leases statistic']
        output = self.fw.do_cli_commands(commands, tag=1)[1]
    
    def show_address_group_trusted_relay_agent(self):
        commands = ['show address-group ipv4 "Default Trusted Relay Agent List"']
        output = self.fw.do_cli_commands(commands, tag=1)[1]
        return output   
...
class DNSCli:
    ...
    # support over 7.1.1 version
    def add_white_list_domains(self, *domains):
        commands = ['configure', 'dns-security']
        for domain in domains:
            commands.append('white-list-entry ' + domain)
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result
    ...
    # support over 7.1.1 version
    def del_white_list_domains(self, *domains):
        commands = ['configure', 'dns-security']
        if not domains:
            commands.append('no white-list-entries')
        else:
            for entry in domains:
                commands.append('no white-list-entry ' + entry)
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result

    def add_edit_filtering_custom_domain(self, tag=0, domain='', category=''):
        commands = ['configure', 'dns-security', 'dns-filtering']
        if domain and category:
            commands.append(f'custom-domain "{domain}"')
            commands.append(f'category "{category}"')
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        if tag == 1:
            (result, msg) = self.fw.do_cli_commands(commands, tag=1)
            return (result, msg)
        result = self.fw.do_cli_commands(commands)
        return result
    
    def del_filtering_custom_domain(self, tag=0, domain=''):
        commands = ['configure', 'dns-security', 'dns-filtering']
        if domain:
            commands.append(f'no custom-domain "{domain}"')
        else:
            commands.append('no custom-domains')
        for command in ['commit', 'exit']:
            commands.append(command)
        if tag == 1:
            (result, msg) = self.fw.do_cli_commands(commands, tag=1)
    ...
    def show_filtering_custom_domain(self, domain=''):
        commands = []
        if domain:
            commands.append(f'show dns-security dns-filtering custom-domain "{domain}"')
        else:
            commands.append('show dns-security dns-filtering custom-domains')
        result = self.fw.do_cli_commands(commands, tag=1)[1]
        return result

    def delete_dns_proxy_static_cache_entry(self, *name):
        commands = ['configure', 'dns-proxy']
        if not name:
            commands.append('no cache-entries')
        else:
            for entry in name:
                commands.append('no cache-entry ' + entry)
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result
...
class ARPCli:
    ...
    def flush_arp_entry(self, *entries):
        commands = ['configure', 'arp']
        if 'all' in entries:
            commands.append('clear arp cache entries')
        else:
            for each in entries:
                commands.append('clear arp cache entry ' + each)
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands)
        return result
...
class IpHelperCli():
    ...
    def edit_relay_policies(self, org_policy, change_policy):
        # org_policy is {
        #     'protocol': 'DHCP',
        #     'from': 'interface X0',
        # }
        # change_policy is {'to-new': 'name "X2 IP"'}
        commands = ['configure', 'ip-helper']
        if 'protocol' in org_policy.keys() and 'from' in org_policy.keys():
            commands.append(f'policy protocol {org_policy["protocol"]} source {org_policy["from"]}')
        else:
            logger.info('old policy dict error.')
            return False
        self.fw._is_key_exist(commands, change_policy, 'enable-new', key_new='enable')
        self.fw._is_key_exist(commands, change_policy, 'protocol-new', key_new='protocol')
        self.fw._is_key_exist(commands, change_policy, 'source-new', key_new='source')
        self.fw._is_key_exist(commands, change_policy, 'to-new', key_new='destination')
        for command in ['end', 'commit', 'exit']:
            commands.append(command)
        logger.info(commands)
        result = self.fw.do_cli_commands(commands)
        ...
        if 'guest-services' in kwargs.keys() and kwargs['guest-services']:
            ...
            if 'bypass-guest-auth' in kwargs.keys() and kwargs['bypass-guest-auth'] :
                if 'bypass-guest-auth-type' in kwargs.keys() and kwargs['bypass-guest-auth-type'] != None:
                    ...
                    return False
            if 'smtp-redirect' in kwargs.keys() and kwargs['smtp-redirect']:
            ...
            if 'deny-networks' in kwargs.keys() and kwargs['deny-networks']:
                if 'version' in kwargs.keys() and kwargs['version'] == 'ipv6':
                ...
                if 'deny-networks-type' in kwargs.keys() and kwargs['deny-networks']:
                    ...
                    return False
            if 'pass-networks' in kwargs.keys() and kwargs['pass-networks']:
                if 'version' in kwargs.keys() and kwargs['version'] == 'ipv6':
...
class RouteCli():
    ...
    def add_route_policy(self, **kwargs):
        ...
        self.fw._is_key_exist(commands, kwargs, 'application')
        self.fw._is_key_exist(commands, kwargs, 'comment')
        ...
        #start for ecmp
        self.fw._is_key_exist(commands, kwargs, 'nexthop-number')
        self.fw._is_key_exist(commands, kwargs, 'interface2')
        self.fw._is_key_exist(commands, kwargs, 'gateway2')
        self.fw._is_key_exist(commands, kwargs, 'interface3')
        self.fw._is_key_exist(commands, kwargs, 'gateway3')
        self.fw._is_key_exist(commands, kwargs, 'interface4')
        self.fw._is_key_exist(commands, kwargs, 'gateway4')
        self.fw._is_key_exist(commands, kwargs, 'gateway')
        #end for ecmp
        self.fw._is_key_exist(commands, kwargs, 'tos')
        self.fw._is_key_exist(commands, kwargs, 'mask')
        commands.extend(['end', 'commit', 'exit'])
        result = self.fw.do_cli_commands(commands)
        return result         
    ...
    def edit_sdwan_route_policy_source(self, **kwargs):  # added for editing sdwan source route policy
        commands = ['configure']
        version = _get_version(kwargs)
        if 'source-new-name' in kwargs.keys() and kwargs['source-new-name']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source name ' + kwargs['source-new-name'])
        if 'source-new-group' in kwargs.keys() and kwargs['source-new-group']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source group ' + kwargs['source-new-group'])
        if 'source-new-network' in kwargs.keys() and kwargs['source-new-network']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source group ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source network ' + kwargs['source-new-network'])
        if 'source-new-range' in kwargs.keys() and kwargs['source-new-range']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source network ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source range ' + kwargs['source-new-range'])
        if 'source-new-host' in kwargs.keys() and kwargs['source-new-host']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source range ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source host ' + kwargs['source-new-host'])
        if 'source-new-any' in kwargs.keys() and kwargs['source-new-any']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source host ' + kwargs['source'] + ' service ' + kwargs[
                                'service'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source ' + kwargs['source-new-any'])
        if 'source-new-name-app' in kwargs.keys() and kwargs['source-new-name-app']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' source ' + kwargs['source'] + ' app ' + kwargs['app'] + ' path-selection-profile ' +
                            kwargs['path-selection-profile'])
            commands.append('source name ' + kwargs['source-new-name-app'])
        if 'source-new-name-application' in kwargs.keys() and kwargs['source-new-name-application']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs['metric']+ ' ' +' source ' + kwargs['source'] + ' application '+ kwargs['application'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('source name ' + kwargs['source-new-name-application'])
        if 'destination-new-name-app' in kwargs.keys() and kwargs['destination-new-name-app']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs[
                'metric'] + ' ' + ' destination ' + kwargs['destination'] + ' app ' + kwargs[
                                'app'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('destination name ' + kwargs['destination-new-name-app'])
        if 'destination-new-name-application' in kwargs.keys() and kwargs['destination-new-name-application']:
            commands.append('route-policy ' + version + ' interface ' + kwargs['if'] + ' metric ' + kwargs['metric']+ ' ' +' destination ' + kwargs['destination'] + ' application '+ kwargs['application'] + ' path-selection-profile ' + kwargs['path-selection-profile'])
            commands.append('destination name ' + kwargs['destination-new-name-application'])   
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
    ...
    def _get_policy(self, **kwargs):
        ...
        if 'application' in kwargs.keys():
            command += 'application' + ' ' + kwargs['application'] + ' '
        if 'gateway' in kwargs.keys():
            command += 'gateway ' + kwargs['gateway'] +  ' '
        else:
            command += 'gateway default' + ' '
        if 'path-selection-profile' in kwargs.keys():
            command += 'path-selection-profile' + ' ' + kwargs['path-selection-profile'] + ' '
        return command
    ...
    def get_route_uuid_by_name(self,version,name):
        uuid=''
        route_found=''
        route_all=self.show_route_policies(version)
        for route in route_all.split("route-policy"):                        
            for line in route.splitlines():
                if '--MORE--' in line:
                    line=line[19:]
                if name in line and "name" ==line.split()[0] and name ==line.split()[1]:
                    route_found=route
                    logger.info(f'find policy with name {name} and this route is {route}')
                    break
                
        for line in route_found.splitlines():
            if '--MORE--' in line:
                line=line[19:]
            if "uuid" in line:
                uuid= line.split()[1]
        ...
        if uuid:
            logger.info(f'find policy with name {name} and uuid {uuid}')
            return uuid
        else:
            logger.error(f'Fail to find policy with name {name}')
            return "fail"
                                              
    def del_route_policy_by_uuid(self,version,uuid):
        commands =['configure', 'no route-policy ' + version + ' uuid ' + uuid] 
        commands.extend(['commit','exit'])
        result = self.fw.do_cli_commands(commands)
        return result

    def del_route_policy_by_name(self,version,name):
        uuid=self.get_route_uuid_by_name(version,name)
        commands =['configure', 'no route-policy ' + version + ' uuid ' + uuid] 
        commands.extend(['commit','exit'])
        result = self.fw.do_cli_commands(commands)
        return result
	
    def edit_route_policy_by_name(self,version,name,**new_policy):
        uuid=self.get_route_uuid_by_name(version,name)
        commands =['configure', 'route-policy ' + version + ' uuid ' + uuid]
        self.fw._is_key_exist(commands, new_policy, 'if-new', key_new='interface')
        self.fw._is_key_exist(commands, new_policy, 'metric-new', key_new='metric')
        self.fw._is_key_exist(commands, new_policy, 'source-new', key_new='source')
        self.fw._is_key_exist(commands, new_policy, 'destination-new', key_new='destination')
        ...
        self.fw._is_key_exist(commands, new_policy, 'tcp-acceleration',
                              key_new='tcp-acceleration')  # added to enable/disable tcp acceleration
        self.fw._is_key_exist(commands, new_policy, 'nexthop-number-new',key_new='nexthop-number')
        self.fw._is_key_exist(commands, new_policy, 'interface2-new',key_new='interface2')
        self.fw._is_key_exist(commands, new_policy, 'gateway2-new',key_new='gateway2')
        self.fw._is_key_exist(commands, new_policy, 'interface3-new',key_new='interface3')
        self.fw._is_key_exist(commands, new_policy, 'gateway3-new',key_new='gateway3')
        self.fw._is_key_exist(commands, new_policy, 'interface4-new',key_new='interface4')
        self.fw._is_key_exist(commands, new_policy, 'gateway4-new',key_new='gateway4')
        self.fw._is_key_exist(commands, new_policy, 'gateway-new',key_new='gateway')
        self.fw._is_key_exist(commands, new_policy, 'nexthop-number')
        self.fw._is_key_exist(commands, new_policy, 'interface2')
        self.fw._is_key_exist(commands, new_policy, 'gateway2')
        self.fw._is_key_exist(commands, new_policy, 'interface3')
        self.fw._is_key_exist(commands, new_policy, 'gateway3')
        self.fw._is_key_exist(commands, new_policy, 'interface4')
        self.fw._is_key_exist(commands, new_policy, 'gateway4')
        self.fw._is_key_exist(commands, new_policy, 'gateway')     
        self.fw._is_key_exist(commands, new_policy, 'tos-new', key_new='tos')
        self.fw._is_key_exist(commands, new_policy, 'mask-new', key_new='mask')	 
    ...
    def config_ospf_keychain(self, **kwargs):
        # single keychain，single key
        # single_config = {
        #     'keychains': [
        #         {
        #             'keychain_name': 'test1',
        #             'keys': [{'key': '1', 'algorithm': 'hmac-sha-256'}]
        #         }
        #     ]
        # }

        # # single keychain，multiple keys
        # single_multi_keys = {
        #     'keychains': [
        #         {
        #             'keychain_name': 'test1',
        #             'keys': [
        #                 {'key': '1', 'algorithm': 'hmac-sha-256'},
        #                 {'key': '2', 'algorithm': 'md5'}
        #             ]
        #         }
        #     ]
        ...
        # multiple_config = {
        #     'keychains': [
        #         {'keychain_name': 'test1', 'keys': [{'key': '1', 'algorithm': 'hmac-sha-256'}]},
        #         {'keychain_name': 'test2', 'keys': [{'key': '1', 'algorithm': 'md5'}]}
        #     ]
        # }
        commands = ['configure', 'routing', 'ospf', 'configure terminal']
        for keychain in kwargs['keychains']:
            commands.append(f'key chain {keychain["keychain_name"]}')
            if 'keys' in keychain:
                for key_config in keychain['keys']:
                    logger.info(f'ready to add {key_config} into commands')
                    commands.append(f'key {key_config["key"]}')
                    if 'algorithm' in key_config:
                        commands.append(f'algorithm {key_config["algorithm"]}')
                    if 'key_string' in key_config:
                        commands.append(f'key-string {key_config["key_string"]}')
                    elif 'key_hex_string' in key_config:
                        commands.append(f'key-hex-string {key_config["key_hex_string"]}')
                    for lifetime in ['accept_lifetime', 'send_lifetime']:
                        if lifetime in key_config:
                            start, end = key_config[lifetime].split(',')
                            commands.append(f'{lifetime.replace("_", "-")} {start.strip()} {end.strip()}')
            commands.append('exit')
    ...
    # add by JLian
    def modify_ospf_keychain(self, **kwargs):
        # modify_dict = {
        #     'keychain_name': 'keychain_test',
        #     # 'delete_key': '1',    #  only use when delete key id(0-255)
        #     'key': '1',
        #     'accept_lifetime': '00:00:00 Jan 01 2023, 23:59:59 Dec 31 2023',
        #     'algorithm': 'hmac-sha-512',
        #     'key_string': 'test0123456789',
        #     'remove':['algorithm','send_lifetime','accept_lifetime'] or 'algorithm'
        #}

        """Modify OSPF keychain attributes"""
        commands = ['configure', 'routing', 'ospf', 'configure terminal']
        if 'keychain_name' not in kwargs or not kwargs['keychain_name']:
            logger.error('keychain_name is required')
            return False, 'keychain_name required'
        commands.append(f'key chain {kwargs["keychain_name"]}')

        # delete entire key
        if 'delete_key' in kwargs:
            commands.append(f'no key {kwargs["delete_key"]}')

        # modify/add a key
        elif 'key' in kwargs and kwargs['key']:
            commands.append(f'key {kwargs["key"]}')
            remove_map = {
                'algorithm': 'algorithm',
                'key_string': 'key-string',
                'key_hex_string': 'key-hex-string',
                'accept_lifetime': 'accept-lifetime',
                'send_lifetime': 'send-lifetime',
            }
            remove_attrs = kwargs.get('remove', [])
            if isinstance(remove_attrs, str):
                remove_attrs = [remove_attrs]
            for attr in remove_attrs:
                cli_key = remove_map.get(attr)
                if cli_key:
                    commands.append(f'no {cli_key}')
                else:
                    logger.warning(f'unknown remove attribute: {attr}')

            if 'algorithm' in kwargs:
                commands.append(f'algorithm {kwargs["algorithm"]}')
            if 'key_string' in kwargs:
                commands.append(f'key-string {kwargs["key_string"]}')
            elif 'key_hex_string' in kwargs:
                commands.append(f'key-hex-string {kwargs["key_hex_string"]}')
            if 'accept_lifetime' in kwargs:
                commands.append(f'accept-lifetime {kwargs["accept_lifetime"].split(",")[0]} {kwargs["accept_lifetime"].split(",")[1]}')
            if 'send_lifetime' in kwargs:
                commands.append(f'send-lifetime {kwargs["send_lifetime"].split(",")[0]} {kwargs["send_lifetime"].split(",")[1]}')
        else:
            logger.error('Either delete_key or key must be specified when modifying an ospf keychain')
            return False, 'delete_key or key required'
     ...
     # add by JLian
    def config_ospf_interface_keychain(self, interface, keychain_name=None, enable_auth=None, bind_keychain=None):
        # interface : 'X2'
        # keychain_name='test1'
        # enable_auth=True/False/None
        # bind_keychain=True/False/None
        if not interface:
            logger.error('interface parameter is required')
            return False, 'interface parameter is required'
        commands = [
            'configure', 'routing', 'ospf', 'configure terminal',
            f'interface {interface}'
        ]
        # Control authentication enable/disable
        if enable_auth is True:
            commands.append('ip ospf authentication key-chain')
        elif enable_auth is False:
            commands.append('no ip ospf authentication')
        
        # Control keychain binding
        if bind_keychain is True :
            if not keychain_name:
                logger.error('keychain_name must be provided when bind_keychain')
            commands.append(f'ip ospf authentication-key-chain {keychain_name}')
        elif bind_keychain is False:
            commands.append('no ip ospf authentication-key-chain')
        
        commands.extend(['end', 'write terminal', 'exit', 'end', 'commit', 'exit'])
        res, output = self.fw.do_cli_commands(commands, tag=1)
        return res, output

    def delete_ospf_keychain(self, keychain_name=None, keychain_names=None):
        # keychain_name = 'test1'
        # keychain_names=['test1', 'test2']
        commands = ['configure', 'routing', 'ospf', 'configure terminal']
        if keychain_names:
            for name in keychain_names:
                commands.append(f'no key chain {name}')
        elif keychain_name:
            commands.append(f'no key chain {keychain_name}')
        commands.extend(['end', 'write terminal', 'exit', 'end', 'commit', 'exit'])
        res, output = self.fw.do_cli_commands(commands, tag=1)
        return res, output
    ...
    def config_ospf(self, **kwargs):
        ...
        ##add by ldu
        if 'DefaultRouteOrigination' in kwargs.keys():
            if kwargs['DefaultRouteOrigination']:
                commands.append('debug lsa-target 0.0.0.0 type external origination self')
            else:
                commands.append('no debug lsa-target 0.0.0.0')
        ...
        if 'type' in kwargs.keys() and kwargs['type'].lower() == 'disable':
            try:
    ...
    def config_router_bgp(self, cmd, AS):
        # cmd = ['router bgp 10','bgp router-id 10.10.10.10','network 193.168.1.0/24', 'neighbor 11.1.1.2 remote-as 20','neighbor 11.1.1.2 ebgp-multihop 255']
        # AS: <1-4294967295>
        commands = ['configure', 'routing', 'bgp', 'configure terminal', 'router bgp' + ' ' + str(AS)]
        print(type(cmd))
        if type(cmd) == str:
            commands.append(cmd)
        if type(cmd) == list:
            commands.extend(cmd)
        commands.extend(['end', 'write', 'exit', 'exit'])
        res, output = self.fw.do_cli_commands(commands, tag=1)
        return res, output
    ...
    def show_in_routing_bgp(self, cmd):
        # cmd = 'show ip bgp ipv4 unicast'
        commands = ['configure', 'routing', 'bgp', cmd]
        res, output = self.fw.do_cli_commands(commands, tag=1)
        return res, output
    ...
    def show_ospf3(self, mode=''): ### mode = '' | 'neighbor' | 'database' | 'routes'
        ...
        if mode:
            cmd = cmd + str(mode)
        commands = [cmd]
        logger.info(f'commands is :{commands}')
        result = self.fw.do_cli_commands(commands, tag=1)[1]
        return result
...
class AddressObjectCli():
    ...
    def show_address_objects(self, version='', ao_type=''):
        commands = []
        if version:
            # ao_type is range, host or network
            if ao_type:
                logger.info(f'verison is {version} and ao_type is {ao_type}')
                commands.append('show address-objects ' + version + ' ' + ao_type)
            else:
                commands.append('show address-objects ' + version)
        # [ao_type is mac,fqdn,custom,default]
        else:
            if ao_type:
                commands.append('show address-objects '+ao_type)
            else:
                commands.append('show address-objects')
        logger.info(commands)
        return self.fw.do_cli_commands(commands, tag=1)[1]    
...
class DNSfilteringCli:
    ...
    def show_filtering_base(self):
        commands = ['show dns-security dns-filtering base']
        result = self.fw.do_cli_commands(commands, tag=1)[1]
        return result

    def add_filtering_profile(self, tag=0, **kwargs):
        commands = ['configure', 'dns-security', 'dns-filtering']
        if 'name' in kwargs.keys():
            commands.append('profile ' + kwargs['name'])
        else:
            logger.error('ERR: Please enter profile name!')
            return None
        if 'category' in kwargs.keys():
            for cate in kwargs['category']:
                commands.append('category ' + cate)
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands, tag=tag)
        return result

    def edit_filtering_profile(self, tag=0, **kwargs):
        commands = ['configure', 'dns-security', 'dns-filtering']
        if 'name' in kwargs.keys():
            commands.append('profile ' + kwargs['name'])
        else:
    ...
    def delete_filtering_profile(self, tag=0, name=''):
        commands = ['configure', 'dns-security', 'dns-filtering']
        if name:
            commands.append('no profile ' + name)
        else:
    ...
    def show_filtering_profile(self, name=''):
        commands = ['no cli pager session']
        if name:
            commands.append('show dns-security dns-filtering profile ' + name)
        else:
            commands.append('show dns-security dns-filtering profiles')
        result = self.fw.do_cli_commands(commands, tag=1)[1]
        return result

    def export_dns_filtering_database_csv(self, **kwargs):
        """
        Export command example:
            export dns-filter-db csv ftp ftp://user:password@server/dns-filter-db.csv
            export dns-filter-db csv scp user@server:/home/user/dns-filter-db.csv
        Paramaters:
            kwargs: dict, including protocol, server, user, password. All necessary.
            - protocol: string, export method, value is ftp or scp.
            - server: string, example-192.168.168.201.
            - user: string, username, example-root
            - password: string, password, example-password
            - filepath: string, the filepath of csv file, example-'dnsfilterDB_ftp.csv'
        """
        if not all([kwargs.get(k) for k in ['protocol', 'server', 'user', 'password', 'filepath']]):        
            logger.error('protocol, server, username, password, filepath need to be specified.')
            return False
        self.fw.cli_login()
        # Send command
        command = 'export dns-filter-db csv ' + kwargs['protocol']
        output, ftp = '', ''
        if kwargs['protocol'].lower() == 'ftp':
            ftp = f" ftp://{kwargs['user']}:{kwargs['password']}@{kwargs['server']}/{kwargs['filepath']}"
            command = command + ftp
            output = str(self.fw.do_cli_command(command))
        elif kwargs['protocol'].lower() == 'scp':
            command = command + f" {kwargs['user']}@{kwargs['server']}:/{kwargs['filepath']}"
            output = str(self.fw.do_cli_command(command))
            if re.search(r'password:', output, re.M | re.I):
                output = output + str(self.fw.do_cli_command(kwargs['password']))
        # Check result
        if 'upload complete' in output.lower():
            logger.info('Upload passed.')
            return True
        if 'no changes made' in output.lower():
...
class DNSPolicyCli:
    ...
    def _dns_policy_options(self, **kwargs):
        commands = []
        self.fw._is_key_exist(commands, kwargs, 'name-new', key_new='name')
        self.fw._is_key_exist(commands, kwargs, 'enable')
        self.fw._is_key_exist(commands, kwargs, 'from')      
        self.fw._is_key_exist(commands, kwargs, 'proxy-mode')      
        self.fw._is_key_exist(commands, kwargs, 'comment')
        self.fw._is_key_exist(commands, kwargs, 'connection-limit-enable', key_new='connection-limit source enable')
        self.fw._is_key_exist(commands, kwargs, 'connection-limit-threshold', key_new='connection-limit source threshold')
        self.fw._is_key_exist(commands, kwargs, 'max-connections')
        for opt in ('service', 'action', 'source', 'priority', 'schedule', 'ticket'):
            if opt in kwargs.keys():
                commands.append(opt + ' ' + kwargs[opt])
        return commands
    
    def add_dns_policy(self, tag=0, **kwargs):
        commands = ['configure']
        if 'name' in kwargs.keys():
            commands.append('dns-policy name ' + kwargs['name'])
        elif 'uuid' in kwargs.keys():
            commands.append('dns-policy uuid ' + kwargs['uuid'])
        else:
            logger.error('ERR: Please enter profile name!')
            return None     
        commands += self._dns_policy_options(**kwargs)
        for command in ['commit', 'end', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands, tag=tag)
        return result

    def edit_dns_policy(self, tag=0, **kwargs):
        commands = ['configure']
        if 'name' in kwargs.keys():
            commands.append('dns-policy name ' + kwargs['name'])
        elif 'uuid' in kwargs.keys():
    ...
    def delete_dns_policy(self, tag=0, name='', uuid=''):
        commands = ['configure']
        if name:
            commands.append('no dns-policy name ' + name)
        elif uuid:
            commands.append('no dns-policy uuid ' + uuid)
        else:
            commands.append('no dns-policies')
        for command in ['commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands, tag=tag)
        return result

    def show_dns_policy(self, name='', uuid=''):
        commands = ['no cli pager session']
        if name:
            commands.append('show dns-policy name ' + name)
        elif uuid:
            commands.append('show dns-policy uuid ' + uuid)
        else:
            commands.append('show dns-policies')
        result = self.fw.do_cli_commands(commands, tag=1)[1]
        return result
    
    def clone_dns_policy(self, tag=0, **kwargs):
        commands = ['configure']
        if all([x in kwargs.keys() for x in ['name', 'new-name', 'location']]):
            commands.append(f'clone dns-policy {kwargs["name"]} new-name {kwargs["new-name"]} location {kwargs["location"]}')
        else:
            logger.error('ERR: Please enter valid policy name/new-name/location!')
            return None            
        for command in ['commit', 'exit']:
            commands.append(command)
        result = self.fw.do_cli_commands(commands, tag=tag)
