...
class SdwanGroupCli:
    ...
    def config_sdwan_group(self, **kwargs):
        ...
        if 'group' in kwargs.keys():
            while grp < len(kwargs['group']):
                commands.append('group ' + kwargs['group'][grp])
                if 'interface' in kwargs.keys():
                    temp = 0
                    iface = 0
                    #while temp < len(kwargs['interface']):
                    while iface < len((kwargs['interface'][temp])):
                            if re.findall(r"^x\d{1,2}", (kwargs['interface'][temp][iface])):
                                #print("debug111")
                                #print(kwargs['interface'][temp][iface])
                                commands.append('interface ' + kwargs['interface'][temp][iface])
                        ...
                        #temp +=1
                    temp +=1
                    if 'egress-bandwidth' in kwargs.keys():
                        commands.append('egress-bandwidth' + ' ' + kwargs['egress-bandwidth'])
                    if 'ingress-bandwidth' in kwargs.keys():         
