...
########nat policy###########
'''
from modules.CLI.network NatpolicyCli
nat = NatpolicyCli(fw)
...
edit_nat_by_uuid = {
    'version': 'ipv4', # must define, ipv4, ipv6, nat64 or not defined
    'uuid': '0008f6b6-612a-4542-0800-2cb8ed9d3c20', # must define, the uuid of the nat policy
    'trans_service_type': 'any'
}
# nat.edit_natpolicy_by_uuid(**edit_nat_by_uuid)
...
# dhcp_dict = {'type': ''} # static | dynamic | all/'' -- for both static and dynamic
# dhcpserver.delete_all_scopes(**dhcp_dict)
...
print(routing.conf_BGP('off'))       # # # 'on' | 'off'
print(routing.show_BGP('summary'))   # # # '' | 'neighbor' | 'summary' | 'unicast'

######mac ip anti sproof
from modules.CLI.network import MacIPAntiSproofCli
macip = MacIPAntiSproofCli(fw)
