            ...
            try:
                ...
                if 'interface_name' in kwargs.keys():
                    for access_name in kwargs['interface_name']:
                        ...
                        group_list.append(access_gt)
                        count += 1

                    json_input['sdwan']['group'][0]['interface'] = group_list

                # -------- PRIORITY (Always 1,2,3...) --------
                if 'interface_name' in kwargs.keys():
                    for idx in range(count):
                        json_input['sdwan']['group'][0]['interface'][idx]['priority'] = idx + 1

                # -------- COST --------
                if 'cost' in kwargs.keys():
                    for idx in range(count):
                        json_input['sdwan']['group'][0]['interface'][idx]['cost'] = kwargs['cost']

                # -------- INGRESS BANDWIDTH --------
                if 'ingress_bandwidth' in kwargs.keys():
                    for idx in range(count):
                        json_input['sdwan']['group'][0]['interface'][idx]['ingress_bandwidth'] = kwargs['ingress_bandwidth']

                # -------- EGRESS BANDWIDTH --------
                if 'egress_bandwidth' in kwargs.keys():
                    for idx in range(count):
                        json_input['sdwan']['group'][0]['interface'][idx]['egress_bandwidth'] = kwargs['egress_bandwidth']

            except KeyError:
                logger.info('Error: In Creating the Json for sdwan group interfaces')
...
#Build json for sdwan performance class object Page
    ...
    def edit_sdwan_perf_class_name(self, nam, msg=False, **kwargs):
        self.options = dict(SDWANPerfClassAPI.default_options)
        self.options.update(kwargs)
        kwargs = self.options
        json_input = self.build_json_sdwan_perf_class(**kwargs)
        logger.info('The json input build is',json_input)
        url_probe = self.url + '/' + 'name' + '/'  + nam
        sdwanperf_resp = self.fw.api_put(url_probe, msg, data=json_input)
        return sdwanperf_resp
    ...
    def build_json_sdwan_route(self, **kwargs):
        ...
        try:
            ...
            if 'schedule' in kwargs.keys():
                ...
                # If schedule is a dict like {"always_on": True}
                if isinstance(kwargs['schedule'], dict):
                    json_input['route_policies'][0]['ipv4']['schedule'] = kwargs['schedule']

                # If schedule is string like "always_on"
                elif isinstance(kwargs['schedule'], str):
                    json_input['route_policies'][0]['ipv4']['schedule'] = {
                        kwargs['schedule']: True
                    }

                # If schedule is None or empty → remove schedule
                else:
                    json_input['route_policies'][0]['ipv4'].pop('schedule', None)
