...
class TestConfigPC(Test):
    ...
    def test_01_add_route_to_local_x0_subnet_on_pc3_eth1(self):
        cmds = [f'route add -net {Parameter.X2_SUBNET} netmask {Parameter.MASK} gw {Parameter.X3_REMOTE_IP}',
                'ip -4 r']
        output = PC3_Login.send_commands(cmds)
        flag = True if f'{Parameter.X2_SUBNET}/24 via {Parameter.X3_REMOTE_IP} dev eth1' in output else False
