...
def ping_traffic_blocked():
    ...
    for i in range(10):
        ...
        if '100% packet loss' in str(out):
            logger.info('Ping from local to remote blocked')
            rc = True
            break
        elif i == 9:
            logger.info('Ping from local to remote cannot be blocked')
            logger.info(out)
            rc = False
    return rc


def check_test_log(reg_list):
    logger.info('check Test log')
    time.sleep(3)
    log = str(LogObj.export_log_txt(log_switch=False))
    flag = 0
    for reg in reg_list:
        if re.search(reg, log):
            ...
            logger.info(f'match this failed: {reg}')
    if flag == len(reg_list):
        rc = True
    else:
        rc = False
        logger.info(log)
...
def check_test_log_from_remote(reg_list):
    logger.info('check Test log')
    time.sleep(3)
