...
def check_test_log_list(reg_list):
    ...
    logger.info(log)
    for reg in reg_list:
        if re.search(reg, log, re.I|re.M):
            match += 1
            logger.info("match log: {}".format(reg))
        else:
            logger.info("didn't match log: {}".format(reg))
    if match == len(reg_list):
        rc = True
        logger.info("match logs passed")
    else:
        rc = False
        logger.info("match logs failed")
        logger.info(log)
    return rc
