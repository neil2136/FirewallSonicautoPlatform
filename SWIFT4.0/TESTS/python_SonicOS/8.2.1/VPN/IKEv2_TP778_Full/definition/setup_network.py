...
class Testsetup_network(Test):
    ...
    def test_00_00_define_global_net(self):
        global dut_x0, rm_x0, gw_x0,\
            dut_x0_net, gw_x0_net, rm_x0_net, rm_x1_net, rm_x2_net, route_eth0_path
    ...
    def test_00_03_add_route_in_pc3(self):
        logger.info('add route in PC3')
        route_list = [
            'route add -net {}/24 gw {}'.format(rm_x1_net, gw_x0),
            'route add -net {}/24 gw {}'.format(rm_x2_net, gw_x0),
            'echo \"{}/24 via {} dev eth0\" >> {}'.format(rm_x1_net, gw_x0, route_eth0_path),
            'echo \"{}/24 via {} dev eth0\" >> {}'.format(rm_x2_net, gw_x0, route_eth0_path),
        ]
        for route_cmd in route_list:
            PC3_login.send_command(route_cmd)
        resp = PC3_login.send_command('route -n')
        reg1 = re.search('{}\s+{}'.format(rm_x1_net, gw_x0), str(resp), re.I|re.M)
        reg2 = re.search('{}\s+{}'.format(rm_x2_net, gw_x0), str(resp), re.I|re.M)
        if reg1 and reg2:
            rc = True
        else:
            rc = False
