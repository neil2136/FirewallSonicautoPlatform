...
from definition.vpn_page_analysis import tunnel_statistics


@paramunittest.parametrized(
    {'options': 'TRAFFIC',  'uuid': '1509730', 'tcid': '2'},
    {'options': 'ICON',  'uuid': '1509729', 'tcid': '1'},
    {'options': 'RENEGOTIATE',  'uuid': '1509732', 'tcid': '5'},
)
class TestVPN_Tunnel_Statistics_01(Test):

    def setParameters(self, options, uuid, tcid):
        self.option = options
        self.uuid = uuid
        self.tcid = tcid
    ...
    @repeat_method(3)
    def test_01_04_Test_VPN_Tunnel_Statistics(self):
        logger.info(" {} ".center(20, '-').format('Test VPN Tunnel Statistics'))
        rc = tunnel_statistics(self.option)
        Assertion.assert_equal(rc, True, "ERR: Test VPN Tunnel {} statitics failed".format(self.option))
