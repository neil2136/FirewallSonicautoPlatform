...
if __name__ == '__main__':
    ...
    parser.add_argument('-turl', type=str, dest='targeturl', required=False, help='target page need check')
    ...
    ui_obj = FWPage(args.url, args.targeturl, args.user, args.pwd)
    res = ui_obj.login_fw_ui()
    print(f'login fw result is {res}')
    if args.targeturl:
        # target url 'http://12.12.1.50/virus'
        source = ui_obj.get_a_page_source()
        logger.info(json.dumps(source))
