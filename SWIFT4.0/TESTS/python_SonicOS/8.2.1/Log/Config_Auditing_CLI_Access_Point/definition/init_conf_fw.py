...
class TestInitConfigFW(Test):
    ...
    def test_03_add_address_object(self):
        #used for add syslog server
        rc = ao_api.config_addressobject(**pc1_ao_dict)
    ...
    def test_08_add_wan_vlan_to_x3(self):
        logger.info("config a wan zone vlan interface to x3... ")
        rc = interface_api.add_interface(**x3_vlan1_dict)
        Assertion.assert_equal(rc, True, "ERR: Config wan zone vlan interface to x3 failed")
