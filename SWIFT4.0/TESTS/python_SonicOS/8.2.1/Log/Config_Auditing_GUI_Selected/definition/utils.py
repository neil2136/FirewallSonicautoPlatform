...
def check_result(check_list: list, output, mode: str = None):
    ret = False
    rc = []
    try:
        if check_list:
...
def verify_result(tcnum: int, mode: str, checklist: list = None):
    rc = False
    if mode == 'syslog':
        time.sleep(3)
        ...
        logger.info(syslog)
        rc = check_result(
            check_list=check_info_dict['tc' + str(tcnum)], output=syslog, mode='syslog')
    elif mode == 'snmp':
        output = PC2_login.send_command(
            'cat /var/log/messages | grep snmptrapd')
        logger.info(output)
        rc = check_result(
            check_list=check_info_dict['tc' + str(tcnum)], output=output, mode='snmp')
    elif mode == 'audit':
        output = audit_log_api.export_audit_log_txt()
        logger.info(output)
        rc = check_result(check_list=checklist, output=output)
    elif mode == 'log':
        output = log_api.export_log_txt()
        logger.info(output)
        rc = check_result(
            check_list=check_info_dict['tc' + str(tcnum)], output=output)
    return rc
