...
class FWPage(Browser):
    ...
    def login_fw_ui(self):
        try:
            ...
            self.set_text_field(
                "class", "sw-textfield__wrapper__input--with-icon-suffix", self.password
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-url", type=str, dest="url", required=True, help="fw https login url"
    )
    parser.add_argument(
        "-user",
        type=str,
        dest="user",
        required=True,
        help="user to login guest service",
    )
    parser.add_argument(
        "-pwd", type=str, dest="pwd", required=True, help="pwd to login guest service"
    )
