...
class TestConfigPC(Test):
    ...
    def test_02_check_dns_server(self):
        ...
        ping = diag_api.diag_ping(dn=dns_server)
        logger.info(f'Diag ping action...... {ping}')
        out = diag_api.get_Ping_Result()
        Assertion.assert_regular(json.dumps(out), f'is alive', '==> ERR: Cannot access to dns server')
