...
class TestConfigPC(Test):
    ...
    def test_00_01_set_pc1_route_ipv4(self):
        ...
        logger.info(result)
        Assertion.assert_regular(result, 'default via 192.168.168.168',
                                 "ERR: Set route IPv4 on PC1 failed")
    ...
    def test_00_02_set_pc3_route_ipv4(self):
        ...
        PC3_login.send_command(
            "route add -net 172.20.1.0 netmask 255.255.255.0 gw " +
            Parameter.FIREWALL)
        PC3_login.send_command(
            "route add -net 172.30.1.0 netmask 255.255.255.0 gw " +
            Parameter.FIREWALL)
        result = os.popen('ip -4 r').read()
        logger.info(result)
        Assertion.assert_regular(result, 'default via 192.168.168.168',
                                 "ERR: Set route IPv4 on PC4 failed")
