...
class TestConfigTB(Test):
    ...
    def test_05_enable_DNS_Filtering_part_on_log_settings(self):
        ...
        for i in eventid:
            ...
            rc = log_set.edit_event(event_id=str(i), **event)
        Assertion.assert_equal(rc, True, "ERR: Enable_gui_button_of_DNS_Filtering_part_on_log_settings failed")
        
    def test_06_set_log_level_to_inform(self):
        rc = log_cata.logging_level(level='inform')
        Assertion.assert_equal(rc, True, "ERR: Set_log_level_to_inform failed")
    
    def test_07_set_Display_Events_in_Log_Monitor_to_0_seconds(self):
        set_log_monitor = {
            "log": {
                "category": [
                    {
    ...
    def test_08_config_dns_security_logging(self):
        log_id = 107
        log_group_dict = {
            "log":{
                "group":[
