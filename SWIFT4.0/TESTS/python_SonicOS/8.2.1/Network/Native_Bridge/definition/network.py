...
class DnsSettingsApi:
    ...
    def edit_split_dns(self, msg=False, **kwargs):
        ...
        kwargs = self.options 
        json_input = self.build_json_split_dns(**kwargs)
        split_dns_resp = self.fw.api_put(self.split_dns_url, msg, data=json_input)
        return split_dns_resp
...
class IpHelperApi:
    ...
    def add_protocol(self, msg=False, **kwargs):
        ...
        json_input = self.get_protocol(protocol='All')
        json = copy.deepcopy(self.initial_protocol_json)
        ...
        json['name'] = kwargs['name']
        json['port1']['value'] = kwargs['port1']
        json['port2']['value'] = kwargs['port2']
        json['timeout'] = kwargs['timeout']
        if 'enable' in kwargs.keys():
            json['enable'] = kwargs['enable']
        json_input["ip_helper"]['protocol'].append(json)
        logger.info(json_input)
        rc = self.fw.api_put(url, msg, data=json_input)
        return rc
...
class DnsPolicyApi:
    ...
    def build_DnsProxyPolicy_json(self, **kwargs):
        ...
        if 'source' in kwargs.keys():
            json_input['dns_policies'][0]['source']=kwargs['source'].lower()
