...
import pexpect
import time
sys.path.append(os.environ["PYTHON_SONICOS_HOME"] + '/User/CLDR-Feature/Compromised_Pwd_Check_In_WAN/')
from definition.settings import *
from definition.conf_fw import *
from pexpect import pxssh
from definition import users

def verify_connection_SSH(openstack_PC, ip, username, password):
    try:
        s = pxssh.pxssh()
        ...
        if i == 0:
            # First-time connection, accept fingerprint
            s.sendline("yes")
            s.expect("Password:")
        ...
        # Capture output after login
        s.prompt(timeout=10)
        output = s.before.decode("utf-8", errors="ignore")
        logger.info(output)

        # Detect prompt and warnings
        ...
        # ---- Decision Logic ----
        if access_denied:
            return {"status": "FAIL_ACCESS_DENIED", "output": output}

        if warning_present and prompt_ok:
            return {"status": "SUCCESS_WITH_WARNING", "output": output}

        if warning_present and not prompt_ok:
            return {"status": "FAIL_NO_PROMPT", "output": output}
        ...
        return {"status": "SSH_FAILURE", "output": str(e)}
...
# FOR LDAP USER
class TC01_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77841"
    description = show_testcase_info(Parameter.TESTPLAN, '4405989', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405989')
    ...
    def test_02_user_login(self):
        headers = OrderedDict([('Accept', 'application/json'),
                               ...
                               ('charset', 'UTF-8')])
        Local_User = users.UserLoginApi(headers, Parameter.X1_IP, 'test', 'password')
        time.sleep(10)
...
class TC02_compromised_pwd_check_in_wan(Test):
    uuid = '405988'
    description = show_testcase_info(Parameter.TESTPLAN, '405988', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '405988')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
class TC03_compromised_pwd_check_in_wan(Test):
    # CLI Case
    uuid = "SOSAIOT-TC-77839"
    description = show_testcase_info(Parameter.TESTPLAN, '4405987', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405987')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    ...
    def test_02_user_login_via_Cli(self):
        result = verify_connection_SSH("-PC1", Parameter.X1_IP, "test", "password")
...
class TC04_compromised_pwd_check_in_wan(Test):
    # CLI Case
    uuid = "SOSAIOT-TC-77838"
    description = show_testcase_info(Parameter.TESTPLAN, '4405986', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405986')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
# FOR RADIUS USER
class TC05_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77845"
    description = show_testcase_info(Parameter.TESTPLAN, '4405993', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405993')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
class TC06_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77844"
    description = show_testcase_info(Parameter.TESTPLAN, '4405992', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405992')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
class TC07_compromised_pwd_check_in_wan(Test):
    # CLI Case
    uuid = "SOSAIOT-TC-77843"
    description = show_testcase_info(Parameter.TESTPLAN, '4405991', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405991')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
class TC08_compromised_pwd_check_in_wan(Test):
    # CLI Case
    uuid = "SOSAIOT-TC-77842"
    description = show_testcase_info(Parameter.TESTPLAN, '4405990', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405990')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
# FOR TACACS USER
class TC09_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77849"
    description = show_testcase_info(Parameter.TESTPLAN, '4405997', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405997')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    ...
    def test_03_user_login(self):
        headers = OrderedDict([('Accept', 'application/json'),
                               ...
                               ('Accept-Encoding', 'application/json'),
                               ('charset', 'UTF-8')])
        Local_User = users.UserLoginApi(headers, Parameter.X1_IP, 'user1', 'password')
        time.sleep(10)
        is_auth, bearer_token = Local_User.local_user_login()
        Assertion.assert_regular(is_auth,
                                 r"(?i)the password is one that is known to have possibly been compromised, it must be changed before you can log in",
...
class TC10_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77848"
    description = show_testcase_info(Parameter.TESTPLAN, '4405996', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405996')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
...
class TC11_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77847"
    description = show_testcase_info(Parameter.TESTPLAN, '4405995', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405995')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    ...
    def test_03_user_login_via_Cli(self):
        result = verify_connection_SSH("-PC1", Parameter.X1_IP, "user1", "password")
        Assertion.assert_equal(
            result["status"],
            "FAIL_ACCESS_DENIED",
            "ERR: Login should be blocked when compromised password check is enabled.")

class TC12_compromised_pwd_check_in_wan(Test):
    uuid = "SOSAIOT-TC-77846"
    description = show_testcase_info(Parameter.TESTPLAN, '4405994', description=True)['title']

    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '4405994')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
