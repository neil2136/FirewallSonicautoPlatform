...
class TestConfigTB(Test):
    ...
    def test_07_remote_add_lan_vpn_acl(self):
        # output = r_accessrules.reset_accessrule_default_setting()
        access_rule = {
            'name': 'Rule1',
            'from': 'LAN',
            'to': 'VPN',
    ...
    def test_09_Add_Network_Obj(self):
        # rc = r_address_groups.del_addressgroup(local_group["name"]) #remove
        # rc &= r_address_objects.del_ao_by_name(remote_vpn_lan["name"], "ipv4")
        # rc &= r_address_objects.del_ao_by_name(local_vpn_obj["name"], "ipv4")
        # rc &= r_address_objects.del_ao_by_name(local_wan_host["name"], "ipv4")
        # rc &= r_address_objects.del_ao_by_name(remote_vpn_dmz["name"], "ipv4")
        # rc &= r_address_objects.del_ao_by_name(remote_wan_host["name"], "ipv4")
        # rc = r_address_groups.del_addressgroup(remote_group["name"]) #remove
        address_objects.del_all_address_object()
        rc = address_objects.config_addressobject(**local_vpn_obj)
        rc &= address_objects.config_addressobject(**local_wan_host)
        ...
        rc &= r_address_objects.config_addressobject(**remote_vpn_dmz)
        rc &= r_address_objects.config_addressobject(**remote_wan_host)
