...
class TestConfigWorkStationClient(Test):
    ...
    def test_01_restart_pcs(self):
        ...
        Assertion.assert_equal(pc2_resp, True, "ERR: Failed to reboot PC2.")
    ...
    def test_03_config_dmz(self):
        # change computer name
        command =(
            "powershell -Command \""
            "Rename-Computer -NewName 'DESKTOP-DMZ' -Force; "
            "Restart-Computer -Force; "
            "\"")
        logger.info(f"Sending command - {command}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command)
        logger.info("Waiting for PC to restart!")
        time.sleep(60)
        start_time = time.time()
        while time.time()-start_time < 60:
            if pc1.ping(Parameter.DMZ_IP):
                break
            time.sleep(10)
    ...
    @repeat_method(2)
    def test_05_config_sso_workstation_DMZ_client(self):
        command1 = (
            "powershell -Command \""
            "Copy-Item -Path 'C:\\ProgramData\\ssh\\sshd_config' -Destination 'C:\\ProgramData\\ssh\\sshd_config.bak'; "
            "(Get-Content -Path 'C:\\ProgramData\\ssh\\sshd_config') -replace '#PasswordAuthentication yes', 'PasswordAuthentication yes' | Set-Content -Path 'C:\\ProgramData\\ssh\\sshd_config'; "
        ...
        logger.info(f"Sending command - {command1}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command1)
        ...
        logger.info(f"Sending command - {command2}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command2)
        
        # restart machine
        command3 = "shutdown /r /f /t 0"
        logger.info(f"Sending command - {command3}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command3)
        logger.info("Waiting for PC to restart!")
        time.sleep(60)
        start_time = time.time()
        while time.time()-start_time < 60:
        ...
        command4 = f"netsh advfirewall set allprofiles state off"
        logger.info(f"Sending command - {command4}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command4)

    def test_06_config_routes(self):
        command1 = f"route add 12.12.1.0 MASK 255.255.255.0 {Parameter.FIREWALL}"
        logger.info(f"Sending command - {command1}")
        ssh_execute_command(Parameter.LAN_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command1)
        command2 = f"route add 12.12.1.0 MASK 255.255.255.0 {Parameter.X2_IP}"
        logger.info(f"Sending command - {command2}")
        ssh_execute_command(Parameter.DMZ_IP, Parameter.WORKSTATION_U, Parameter.WORKSTATION_P, command2)
