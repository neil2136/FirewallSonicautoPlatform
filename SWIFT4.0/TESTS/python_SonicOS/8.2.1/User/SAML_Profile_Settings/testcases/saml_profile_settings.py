...
class Test_TC02(Test):
    ...
    # def test_01_cp_xml_file_to_pc1(self):
    #     pc1_login.send_command(f'\cp {xml_file} /tmp/')
    #     out = pc1_login.send_command('ls -l /tmp')
    #     Assertion.assert_regular(out, 'SAML.xml', 'ERR: cp xml file to pc1 failed!!')
    #
    # def test_02_generate_cert_pem_from_xml_file(self):
    #     rc = generate_pem_cert(file_path='/tmp/certificate.pem')
    #     Assertion.assert_equal(rc, True, 'ERR: generate cert pem file form xml file failed!!')
    #
    # def test_03_import_generated_cert_pem(self):
    #     rc = ca_api.import_ca_cert(file='/tmp/certificate.pem')
    #     Assertion.assert_equal(rc, True, 'ERR: import generated cert file failed!!')

    def test_04_add_idp_profile_from_xml_file(self):
        # auth_url = get_url_from_xml()
        # logger.info(f'auth_url is: {auth_url}')
        # logout_url = get_url_from_xml()
        # logger.info(f'logout_url is: {logout_url}')
        # server_id = get_server_id_from_xml()
        # idp = copy.deepcopy(idp_import)
        # update_opt = {
        #     "authentication_url": auth_url,
        #     "logout_url": logout_url,
        #     'server_id': server_id
        # }
        # idp.update(update_opt)
        # rc = saml_api.add_saml_identify_provider(**idp)
        rc = True
        Assertion.assert_equal(rc, True, "ERR: add sam identity provider from xml file failed!!")
...
class Test_TC03(Test):
    ...
    def test_01_add_idp_profile_without_user_name_attribute(self):
        ...
        rc = (not res) and '"message": "SAML IDP Server Name: IdP username attribute can not be empty"' in json.dumps(
            err_msg)
    ...
    def test_01_01_add_maximum_idp(self):
        ...
        for i in range(50):
            ...
            if not add_res:
                ...
                rc = "SAML IDP Server Name: Number of SAML IdP server reaches the limitation:  No memory available" in json.dumps(
                    err_msg)
                break
        Assertion.assert_equal(rc, True, "ERR: add sam identity provider failed!!")
...
class Test_TC06(Test):
    ...
    def test_02_check_maximum_name_length(self):
        out = saml_api.get_saml_identity_providers()
        logger.info(json.dumps(out))
        Assertion.assert_equal(len(out['user']['saml']['identity_provider'][0]['name']), 63,
                               'ERR: check the idp profile name maxinum length failed!!')
...
class Test_TC19(Test):
    ...
    def test_04_delete_idp_profile_in_use(self):
        ...
        logger.info(json.dumps(err_msg))
        rc = (not res) and "SAML IDP Server Name: IdP profile is in use, cannot be deleted" in json.dumps(
            err_msg)
    ...
    def test_02_add_maximum_sp_profiles(self):
        ...
        for i in range(20):
            ...
            if not res:
                ...
                logger.info(json.dumps(err_msg))
                rc = 'SAML SP Server Name: Number of SAML SP reaches the limitation:  No memory available' in json.dumps(
                    err_msg)
                break
...
class Test_TC29(Test):
    ...
    def test_04_delete_idp_profile_in_use(self):
        ...
        rc = (not res) and 'SAML SP Server Name: SP profile is in use, cannot be deleted' in json.dumps(
            err_msg)
        Assertion.assert_equal(rc, True, 'ERR: check del idp profile in use failed!!')
...
class Test_TC30(Test):
    ...
    def test_01_00_del_saml_profiles(self):
    ...
    def test_01_01_add_sp_name_64_length(self):
        sp = copy.deepcopy(svc_provider)
        sp.update({'name': ''.join(random.choices(string.digits + string.ascii_letters, k=64))})
        rc = saml_api.add_saml_service_provider(**sp)
        Assertion.assert_equal(rc, True, 'ERR: add saml service provider failed!!')

    def test_02_check_sp_length(self):
        out = saml_api.get_saml_service_providers()
        Assertion.assert_equal(len(out['user']['saml']['service_provider'][0]['name']), 63,
                               'ERR: Check the SAML SP profile name boundary failed!!')
    ...
    def test_01_add_maximum_saml_profiles(self):
        for i in range(20):
            ...
            if not res:
                ...
                rc = 'SAML Authentication Profile Name: Number of SAML SP reaches the limitation:  No memory available' in json.dumps(
                    err_msg)
                break
