...
class FWPage_UI(FWPage):
    ...
    def navigate_to_device_tab(self):
        ...
        time.sleep(10)
        self.click_element('xpath', "//span[@class='sw-top-nav-item__label--default sw-flexbox__flex-none'][normalize-space()='Device']")
    ...
    def click_on_existing_radius_accounting_client(self, client):
        logger.info(f"Clicking on existing Radius Accounting Client {client}")
        self.click_element('xpath', f"//div[contains(text(),'{client}')]")
        self.move_to_the_element('xpath', f"//div[contains(text(),'{client}')]")
    ...
    def config_radius_acc_client(self, radius_acc_client, existing_client=None, host_exists=False, match_pass=False,
                                 invalid_fwd_server=False,
                                 save=True, close=False,
                                 invalid_ip=False):
        self.click_on_configur_sso()
        self.click_on_radius_accounting_tab()
        if existing_client:
            self.click_on_existing_radius_accounting_client(client=existing_client)
        else:
        ...
        if "clientHost" in configs:
            if radius_acc_client["clientHost"] == "":
                ...
                if host_exists:
                    ...
                    self.wait_for_page_data_to_be_rendered()
                    Assertion.assert_equal(self.does_page_have_text("already exists"), True,
                                       "ERR: Already exists Error not found")
                    # self.click_element('xpath',
                    #                    "//button[@class='sw-btn sw-btn--light sw-btn--default'][normalize-space()='OK']")
        ...
        if "forwarding" in configs:
            ...
            if "server1" in radius_acc_client["forwarding"]:
                if "ip" in radius_acc_client["forwarding"]["server1"]:
                    self.set_text_field("xpath", "//input[@name='users-lg-act-sel-inactv-ula']",
        ...
        if close:
            self.click_on_close_in_add_radius_acc_client()
        logger.info("===============================Successfully created SSO client ===============================")
        time.sleep(10)
        self.click_element('xpath', "/html[1]/body[1]/div[1]/div[1]/div[1]/div[2]/div[4]/div[2]/div[1]/div[1]/section[1]/div[1]/div[1]/div[2]/div[1]/div[1]/div[1]/div[5]/div[1]/div[1]/span[2]/span[1]")
