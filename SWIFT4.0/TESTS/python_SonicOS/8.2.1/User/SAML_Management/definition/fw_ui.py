...
def manage_part():
    manage_rc = fw_page_ui.does_element_exist('xpath', '//*[text()="Manage"]')
...
def get_session_time():
    ...
    for i in range(5):
        obj_session = fw_page_ui.get_element('xpath', "//div[contains(text(), 'Remaining session time')]")
        session_time = obj_session.text
        if not session_time:
            logger.info(f'get the session time failed at {i+1} time, refresh page')
            fw_page_ui.refresh_browser()
        else:
            break
...
def logout_saml_auth():
    for i in range(3):
        ...
        fw_page_ui.switchToDefaultWindow()
        time.sleep(3)
        # handles = fw_page_ui.get_browser_all_handles()
        # return len(handles) == 1 and 'https://shanghaiqa.com' in fw_page_ui.get_current_browser_url()
        # if fw_page_ui.does_element_exist('xpath', '//button[contains(text(), "Log Back In")]'):
        if fw_page_ui.does_element_exist('xpath', '//button[@type="button"]'):
