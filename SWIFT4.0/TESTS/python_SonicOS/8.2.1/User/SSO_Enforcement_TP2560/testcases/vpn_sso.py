...
def check_active_user_status(name, failure=False):
    ...
    if failure:
        ...
        while time.time()-start_time < 60:
            active_sessions = user_status.show_user_status()
            if name in json.dumps(active_sessions):
                break
            time.sleep(10)
...
class NonTC(Test):
    ...
    def test_04_import_user_from_ldap(self):
        input_data = {
            "user": {
                "local": {
                    "user": [{
    ...
    def test_05_user_logout_log_setting(self):
        user_logout ={
            "log": {
                "event": [
                    {
    ...
    def test_06_enable_security_services_vpn(self):
        vpn = {
            "zones":[
                {
                'name': 'VPN',
        ...
        resp = zone_object.edit_zone_object('VPN', **vpn)
        Assertion.assert_equal(resp, True, "ERR: Failed to enable security service on DMZ.")


class TC_13_SSO_Enforcement_Enabled_On_VPN_Zone_With_Security_Service(Test):
    uuid = "SOSAIOT-TC-75801"
    description = show_testcase_info(Parameter.TESTPLAN, '1516342', description=True)['title']
    
    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '1516342')
    ...
    def test_04_sso_login(self):
        ...
        run_io_tasks_in_parallel([
            lambda:ssh_execute_command(Parameter.VPN_IP, f'{Parameter.DOMAIN}\\{Parameter.DOMAIN_U1}', Parameter.DOMAIN_UP1, f'ping {Parameter.SSO_SERV}', timeout=50),
...
class TC_16_SSO_Enforcement_Disabled_On_VPN_Zone_With_Security_Service(Test):
    uuid = "SOSAIOT-TC-75802"
    description = show_testcase_info(Parameter.TESTPLAN, '1516345', description=True)['title']
    
    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '1516345')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    
    def test_01_disable_sso_enforcement(self):
        response = del_enforce_on_zone('VPN')
        Assertion.assert_equal(response, True, "ERR: Failed to disable SSO enforcement on VPN.")
...
class TC_14_SSO_Enforcement_Enabled_On_VPN_Zone(Test):
    uuid = "SOSAIOT-TC-75812"
    description = show_testcase_info(Parameter.TESTPLAN, '1516343', description=True)['title']
    
    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '1516343')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    ...
    def test_01_enable_sso_enforcement(self):
        ...
        response = add_enforce_on_zone_json(**input_data)
        Assertion.assert_equal(response, True, "ERR: Failed to enable SSO enforcement on VPN.")
...
class TC_17_SSO_Enforcement_Disabled_On_VPN_Zone_With_Access_Rule(Test):
    uuid = "SOSAIOT-TC-75813"
    description = show_testcase_info(Parameter.TESTPLAN, '1516344', description=True)['title']
    
    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '1516344')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
    ...
    def test_04_edit_vpn_lan_rule(self):
        rules = accessrule.get_ipv4_access_rule_given_from_to('VPN', 'LAN')
        vpn_lan_rule ={"access_rules":[{}]}
        logger.info(f"rule - {rules['access_rules'][0]}")
        for rule in rules['access_rules']:
            if "name" in rule['ipv4']['source']['address'] and rule['ipv4']['source']['address']['name'] == 'local_vpn':
                vpn_lan_rule["access_rules"][0] = rule
                break
        vpn_lan_rule["access_rules"][0]['ipv4']['users']['included'] = {'all': True}
        uuid = vpn_lan_rule["access_rules"][0]['ipv4']['uuid']
        resp = accessrule.edit_ipv4_access_rule_by_uuid(uuid, **vpn_lan_rule)
        Assertion.assert_equal(resp, True, 'ERR: Failed to update access rule.')


class TC_15_SSO_Enforcement_Disabled_On_VPN_Zone(Test):
    uuid = "SOSAIOT-TC-75814"
    description = show_testcase_info(Parameter.TESTPLAN, '1516346', description=True)['title']
    
    def test_00_show_testcase_info(self):
        show_testcase_info(Parameter.TESTPLAN, '1516346')
        Assertion.assert_equal(True, True, "ERR: show testcase info failed")
